<section id="list-mobil" class="py-5 mt-5">
    ...
    <style>
        ...
        .swiper-slide .card {
            ...
            /* Efek membal sedikit */
            border: none;
            /* Opsional: hilangkan border bawaan bootstrap biar lebih clean */
        }

        /* Saat slide di-hover (oleh mouse) */
        .swiper-slide:hover .card {
            transform: translateY(-10px) scale(1.02);
            /* Naik ke atas 10px & membesar dikit */
            box-shadow: 0 15px 30px rgba(0, 0, 0, 0.15) !important;
            /* Bayangan makin tebal */
            z-index: 2;
            /* Pastikan dia di atas elemen lain */
        }
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        const swiper3DConfig = {
            ...
            on: {
                ...
                slideChange: function () {
                    // Cari semua slide yang sedang terbuka (flipped) dan tutup kembali
                    const flippedSlides = document.querySelectorAll('.swiper-slide.is-flipped');
                    flippedSlides.forEach(slide => {
        ...
        try {
            ...
            console.error("Swiper All Error:", e);
        }
        ...
        slides.forEach(slide => {
            slide.addEventListener('click', function(e) {
                // Cek jika yang diklik adalah tombol WA, jangan flip (biar link jalan)
                if (e.target.closest('a')) return;

                // Toggle class 'is-flipped' pada slide yang diklik
                this.classList.toggle('is-flipped');

                // Opsional: Jika ingin otomatis menutup kartu lain saat satu dibuka
                slides.forEach(otherSlide => {
                    if (otherSlide !== this) {
                        otherSlide.classList.remove('is-flipped');
                    }
                });
            });
        });
        ...
        <?php if (!empty($categories)): ?>
            <?php foreach ($categories as $cat): ?>
                try {
                    ...
                    console.error("Swiper Cat <?= $cat['id'] ?> Error:", e);
                }
            <?php endforeach; ?>
        <?php endif; ?>
    });
